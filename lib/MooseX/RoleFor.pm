...
This package allows your Moose roles to limit what classes and
objects they may be composed with. This is often not a good
idea - one of the advantages of roles is that they can be
reused in such different contexts.

However, if you search CPAN for "TraitFor" you'll see that it's
quite a common desire to indicate that a role should only be
applies to certain classes.

=head2 C<< role_for $class, $consequence >>

C<< $class >> is a string (class name) or arrayref of strings
indicating which classes this role may be composed with.
...
Adding C<< use MooseX::RoleFor >> to your role imports the
C<role_for> function to your class, and applies the
C<MooseX::RoleFor::Meta::Role::Trait::RoleFor> role to your
role's metaclass.
...
C<Moose::Meta::Role>'s C<apply> method to enforce your
restriction.
